...
for strOption, strArgument in getopt.getopt(sys.argv[1:], '', [ strParameter[2:] + '=' for strParameter in sys.argv[1::2] ])[0]:
    ...
    if strOption == '--seconds' and strArgument != '': arguments_intSeconds = int(strArgument) # path to where the output should be stored
# end
...
if __name__ == '__main__':
    ...
    moviepy.editor.ImageSequenceClip(
            sequence=[npyFrame[:, :, ::-1] for npyFrame in list(reversed(npyResult))],
            fps=fps).write_videofile(arguments_strOut)
# end
